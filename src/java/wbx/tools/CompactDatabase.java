 ...
 *
 *   java -cp "classes:lib/*:conf" wbx.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dwin.runtime.mode=desktop wbx.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize Wbx properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "wbx.testDb" : "wbx.db";
        String dbType = Wbx.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Wbx.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Wbx.getDbDir(Wbx.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Wbx.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Wbx.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Wbx.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "wbx.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "wbx.mv.db");
            if (!dbFile.exists()) {
