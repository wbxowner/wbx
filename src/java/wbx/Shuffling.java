...
import wbx.crypto.AnonymouslyEncryptedData;
...
public final class Shuffling {
    ...
    private static final boolean deleteFinished = Wbx.getBooleanProperty("wbx.deleteFinishedShufflings");
    ...
    private void cancel(Block block) {
        ...
        if (blamedAccountId != 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                Logger.logDebugMessage("Shuffling penalty %f " + Constants.COIN_NAME + " awarded to forger at height %d", ((double)fee) / Constants.ONE_WIN, block.getHeight() - i - 1);
            }
            ...
            Logger.logDebugMessage("Shuffling penalty %f " + Constants.COIN_NAME + " awarded to forger at height %d", ((double)fee) / Constants.ONE_WIN, block.getHeight());
        }
