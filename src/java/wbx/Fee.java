    ...
    abstract class SizeBasedFee implements Fee {
        ...
        public final long getFee(TransactionImpl transaction, Appendix appendage) {
            ...
            if (height <= Constants.NEW_FEE_CALCULATION_BLOCK) {
                constantFee = (this.constantFee < Constants.ONE_WIN ? Constants.ONE_WIN : this.constantFee)/Constants.REDUCTOR_FEE;
            ...
            else constantFee = (this.constantFee < Constants.ONE_WIN ? Constants.ONE_WIN : this.constantFee)/Constants.REDUCTOR_FEE;
            if (this.feePerSize == 0) feePerSize = 0;
            else feePerSize = (this.feePerSize < Constants.ONE_WIN ? Constants.ONE_WIN : this.feePerSize)/Constants.REDUCTOR_FEE;

            fee = (Math.addExact(constantFee, Math.multiplyExact((long) (size / unitSize), feePerSize)));

            if (fee != 0) {
                fee = (fee < Constants.ONE_WIN ? Constants.ONE_WIN : fee)/ Constants.REDUCTOR_FEE;
            }
            return fee;
        }
