...
import static wbx.http.JSONResponses.INCORRECT_TRANSACTION;
import static wbx.http.JSONResponses.MISSING_TRANSACTION;
import static wbx.http.JSONResponses.UNKNOWN_TRANSACTION;
...
public final class GetTransactionBytes extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        transaction = Wbx.getBlockchain().getTransaction(transactionId);
        JSONObject response = new JSONObject();
        if (transaction == null) {
            transaction = Wbx.getTransactionProcessor().getUnconfirmedTransaction(transactionId);
            if (transaction == null) {
                return UNKNOWN_TRANSACTION;
            }
        } else {
            response.put("confirmations", Wbx.getBlockchain().getHeight() - transaction.getHeight());
        }
