...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (Wbx.getBooleanProperty("wbx.enableAPIProxy") && API.openAPIPort == 0 && API.openAPISSLPort == 0);
    private static final int blacklistingPeriod = Wbx.getIntProperty("wbx.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = Wbx.getStringProperty("wbx.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && Wbx.getBlockchainProcessor().isDownloading());
    }

    void blacklistHost(String host) {
        blacklistedPeers.put(host, Wbx.getEpochTime() + blacklistingPeriod);
