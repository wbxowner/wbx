...
public final class PopOff extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        try {
            Wbx.getBlockchainProcessor().setGetMoreBlocks(false);
            if (numBlocks > 0) {
                blocks = Wbx.getBlockchainProcessor().popOffTo(Wbx.getBlockchain().getHeight() - numBlocks);
            } else if (height > 0) {
                blocks = Wbx.getBlockchainProcessor().popOffTo(height);
            } else {
                return JSONResponses.missing("numBlocks", "height");
            }
        } finally {
            Wbx.getBlockchainProcessor().setGetMoreBlocks(true);
        }
        ...
        if (keepTransactions) {
            blocks.forEach(block -> Wbx.getTransactionProcessor().processLater(block.getTransactions()));
        }
        return response;
    }
