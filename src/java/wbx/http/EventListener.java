...
import wbx.AccountLedger;
import wbx.AccountLedger.LedgerEntry;
import wbx.Block;
import wbx.BlockchainProcessor;
import wbx.Db;
import wbx.Wbx;
import wbx.Transaction;
import wbx.TransactionProcessor;
 ...
 * has not been received within wbx.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by wbx.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Wbx.getIntProperty("wbx.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Wbx.getIntProperty("wbx.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Wbx.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Wbx.getTransactionProcessor();
    ...
    /** Wbx event listeners */
    private final List<WinEventListener> winEventListeners = new ArrayList<>();
     ...
     *
     * Wbx event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Wbx event registrations
     ...
     *
     * @param   eventRegistrations      Wbx event registrations
     * @throws  EventListenerException  Invalid Wbx event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<WinEventListener> it = winEventListeners.iterator();
                while (it.hasNext()) {
                    WinEventListener listener = it.next();
                ...
                if (addListener) {
                    WinEventListener listener = new WinEventListener(event);
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            winEventListeners.forEach(WinEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * Wbx event listener
     */
    private class WinEventListener {

        /** Event handler */
        private final WinEventHandler eventHandler;

        /**
         * Create the Wbx event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public WinEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the Wbx listener for this event
         */
        ...
        /**
         * Remove the Wbx listener for this event
         */
        ...
        /**
         * Return the hash code for this Wbx event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Wbx events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof WinEventListener) &&
                    eventHandler.equals(((WinEventListener)obj).eventHandler));
        }

        /**
         * Wbx listener event handler
         */
        private abstract class WinEventHandler {
            ...
            /**
             * Create the Wbx event handler
             *
             * @param   eventRegistration   Event registration
             */
            public WinEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof WinEventHandler) &&
                                        owner == ((WinEventHandler)obj).owner &&
                                        accountId == ((WinEventHandler)obj).accountId &&
                                        event == ((WinEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends WinEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends WinEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends WinEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends WinEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           Wbx listener event
        ...
        /**
         * Return the Wbx listener event
         *
         * @return                  Wbx listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
