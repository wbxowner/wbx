...
import wbx.AccountRestrictions;
import wbx.Alias;
import wbx.Appendix;
import wbx.Asset;
import wbx.AssetDelete;
import wbx.AssetDividend;
import wbx.AssetTransfer;
import wbx.Attachment;
import wbx.Block;
import wbx.Constants;
import wbx.Currency;
import wbx.CurrencyExchangeOffer;
import wbx.CurrencyFounder;
import wbx.CurrencyTransfer;
import wbx.CurrencyType;
import wbx.DigitalGoodsStore;
import wbx.Exchange;
import wbx.ExchangeRequest;
import wbx.FundingMonitor;
import wbx.Generator;
import wbx.HoldingType;
import wbx.MonetarySystem;
import wbx.Wbx;
import wbx.Order;
import wbx.PhasingPoll;
import wbx.PhasingVote;
import wbx.Poll;
import wbx.PrunableMessage;
import wbx.Shuffler;
import wbx.Shuffling;
import wbx.ShufflingParticipant;
import wbx.TaggedData;
import wbx.Token;
import wbx.Trade;
import wbx.Transaction;
import wbx.Vote;
import wbx.VoteWeighting;
import wbx.crypto.Crypto;
import wbx.crypto.EncryptedData;
import wbx.db.DbIterator;
import wbx.peer.Hallmark;
...
public final class JSONData {
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance) {
        return accountBalance(account, includeEffectiveBalance, Wbx.getBlockchain().getHeight());
    }
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance, int height) {
        ...
        if (account == null) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceWIN", "0");
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceWIN", account.getEffectiveBalanceWIN(height));
    ...
    static JSONObject lessor(Account account, boolean includeEffectiveBalance) {
        ...
        if (accountLease.getCurrentLesseeId() != 0) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceWIN", String.valueOf(account.getGuaranteedBalanceNQT() / Constants.ONE_WIN));
            }
        }
    ...
    static JSONObject shuffling(Shuffling shuffling, boolean includeHoldingInfo) {
        ...
        if (includeHoldingInfo && shuffling.getHoldingType() != HoldingType.WBX) {
        ...
        if (includeExecutedPhased) {
            ...
            try (DbIterator<PhasingPoll.PhasingPollResult> phasingPollResults = PhasingPoll.getApproved(block.getHeight())) {
                for (PhasingPoll.PhasingPollResult phasingPollResult : phasingPollResults) {
                    ...
                    if (includeTransactions) {
                        phasedTransactions.add(transaction(Wbx.getBlockchain().getTransaction(phasedTransactionId)));
                    } else {
    ...
    static JSONObject pollResults(Poll poll, List<Poll.OptionResult> results, VoteWeighting voteWeighting) {
        ...
        if (voteWeighting.getMinBalanceModel() == VoteWeighting.MinBalanceModel.ASSET) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = Wbx.getBlockchain().getTransaction(voteWeighting.getHoldingId());
    ...
    static JSONObject transaction(Transaction transaction, Filter<Appendix> filter) {
        ...
        json.put("confirmations", Wbx.getBlockchain().getHeight() - transaction.getHeight());
    ...
    private static void putExpectedTransaction(JSONObject json, Transaction transaction) {
        json.put("height", Wbx.getBlockchain().getHeight() + 1);
        json.put("phased", transaction.getPhasing() != null);
    ...
    static void ledgerEntry(JSONObject json, LedgerEntry entry, boolean includeTransactions, boolean includeHoldingInfo) {
        ...
        if (includeTransactions && entry.getEvent().isTransaction()) {
            Transaction transaction = Wbx.getBlockchain().getTransaction(entry.getEventId());
