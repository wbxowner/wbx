...
var NRS = (function(NRS, $) {
	var depositButtonCache = '';
    var DEPOSIT_ADDRESSES_KEY = "shapeshift.depositAddresses.";
    var SUPPORTED_COINS = {};

    var coinToPair = function (op, coin) {
        return (op == "buy") ? "WBX_" + coin : coin + "_WBX";
    };

    var pairToCoin = function (pair) {
        if (pair.indexOf("WBX_") == 0) {
            return pair.substring("WBX_".length);
        }
        if (pair.indexOf("_WBX") == pair.length - "_WBX".length) {
            return pair.substring(0, pair.indexOf("_WBX"));
        }
    ...
    var renderExchangeTable = function (op) {
        ...
        async.series(tasks, function (err, results) {
            var table = $("#p_shape_shift_" + op + "_wbx");
            if (err) {
        ...
        apiCall('recenttx/50', {}, 'GET', function (data) {
            ...
            if (data) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (String(transaction.curIn).escapeHTML() != "WBX" && String(transaction.curOut).escapeHTML() != "WBX") {
                        continue;
                    }
                    rows += "<tr>";
    ...
    function renderWinLimit() {
        apiCall('limit/wbx_btc', {}, 'GET', function (data) {
            NRS.logConsole("limit1 " + data.limit);
            if (data.limit) {
                $('#shape_shift_status').html('ok');
                $('#shape_shift_wbx_avail').html(String(data.limit).escapeHTML());
            }
        });
    }
        ...
        apiCall('getcoins', {}, 'GET', function (data) {
            ...
            for (var i = 0; i < inputFields.length; i++) {
                ...
                $.each(data, function (code, coin) {
                    if (code != 'WBX' && coin['status'] == 'available') {
    ...
    function loadDepositCoins(parentSelector) {
        // barton allow multiple sets of buttons to mark themselves as loaded based on the elementFromPoint
        //        the buttons are spawned into
        if(!parentSelector)
          parentSelector = "#exchange_deposit_div_button";
        var exchange_deposit_div_button = $(parentSelector);
        //suse dynamically create exchange deposit button based on existing asset in the blockchain
        NRS.sendRequest("getAssetIds", {
            "asset": 'all'
        }, function (response) {
            exchange_deposit_div_button.html('');
    ...
    // function getDepositCoinButton(parentId) {
        ...
        // newButton.push('<div style="margin-bottom:5px" id="' + parentId + '"></div>');
        // loadDepositCoins('#' + parentId);
        // return newButton;
    // }
    ...
    $("#deposit_coin_modal").on("show.bs.modal", function (e) {
        ...
        switch (coin.toLowerCase()) {
            case 'btc':
            case 'win':
                agentEmail = 'win@wdynasty.com';
                break;
            default:
                agentEmail = coin.toLowerCase()+"@wdynasty.com";        
        } 
        
        $("#deposit_coin_modal_title").html(title);
        //$("#exchange_deposit_mail").html($.t("exchange_deposit_mail", { coin: coin, agentEmail: "wbx_agent@email.com" }));
    ...
    NRS.pages.exchange = function() {
        ...
        //var coin = 'BTC';
        //var agentEmail = 'wbx_agent@email.com';
        //$("#exchange_deposit_mail").html($.t("exchange_deposit_mail", { coin: "BTC", agentEmail: "wbx_agent@email.com" }));
        loadDepositCoins();
    ...
    NRS.getFundAccountLink = function() {
        var parentId = 'fund_account_buttons_container';
        var html = '<div style="margin-bottom:5px;" id="' + parentId + '"></div>';
        setTimeout(function() {
          loadDepositCoins('#' + parentId, true);
        }, 0);
        return html;
        ...
        //return "<div class='callout callout-danger'>" +
            ...
            //"data-pair='BTC_WBX'>" + $.t("fund_account_message") + "</a>";
            //return getDepositCoinButton('fund_account_buttons_container').join(" ");
    };
    ...
    $("#m_shape_shift_buy_modal").on("show.bs.modal", function (e) {
        ...
        $("#m_shape_shift_buy_title").html($.t("exchange_wbx_to_coin_shift", { coin: coin }));
        $("#m_shape_shift_buy_min").val(invoker.data("min"));
        $("#m_shape_shift_buy_min_coin").html("WBX");
        $("#m_shape_shift_buy_max").val(invoker.data("max"));
        $("#m_shape_shift_buy_max_coin").html("WBX");
        $("#m_shape_shift_buy_rate").val(invoker.data("rate"));
        $("#m_shape_shift_buy_rate_text").html("WBX/" + coin);
    ...
    $("#m_send_amount_buy_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_buy_title").html($.t("exchange_wbx_to_coin_send_amount", { coin: coin }));
        $("#m_send_amount_buy_withdrawal_amount_coin").html(coin);
        $("#m_send_amount_buy_rate_text").html("WBX/" + coin);
        ...
        apiCall('sendamount', {
            ...
            try {
                ...
                if (amount != data.success.withdrawalAmount) {
                    NRS.showModalError("amount returned from WBX exchange " + data.success.withdrawalAmount +
                    " differs from requested amount " + amount, modal);
                    return;
                }
                if (withdrawal != data.success.withdrawal) {
                    NRS.showModalError("withdrawal address returned from WBX exchange " + data.success.withdrawal +
                ...
                // add 1 WBX fee to make sure the net amount is what requested by shape shift
    ...
    $("#m_shape_shift_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_shape_shift_sell_title").html($.t("exchange_coin_to_wbx_shift", { coin: coin }));
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                $("#m_shape_shift_sell_rate_text").html(coin + "/WBX");
                $("#m_shape_shift_sell_fee").val(data.fee);
                $("#m_shape_shift_sell_fee_coin").html("WBX");
    ...
    $("#m_send_amount_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_sell_title").html($.t("exchange_coin_to_wbx_send_amount", { coin: coin }));
        $("#m_send_amount_sell_rate_text").html("WBX/" + coin);
        $("#m_send_amount_sell_fee_coin").html("WBX");
        $("#m_send_amount_sell_withdrawal_amount_coin").html("WBX");
