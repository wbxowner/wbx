...
var NRS = (function(NRS, $, undefined) {
    ...
    NRS.loadTransactionTypeConstants = function(response) {
        ...
        if (NRS.defaultModules['send_token'] === false) {
        	delete NRS.transactionTypes[0];
        }
        if (NRS.defaultModules['aliases'] === false) {
        	delete NRS.transactionTypes[1]["subTypes"][1];
        	delete NRS.transactionTypes[1]["subTypes"][6];
        	delete NRS.transactionTypes[1]["subTypes"][7];
        	delete NRS.transactionTypes[1]["subTypes"][8];
        }
        if (NRS.defaultModules['polls'] === false) {
        	delete NRS.transactionTypes[1]["subTypes"][2];
        	delete NRS.transactionTypes[1]["subTypes"][3];
        	delete NRS.transactionTypes[1]["subTypes"][4];
        }
        if (NRS.defaultModules['asset_exchange'] === false) delete NRS.transactionTypes[2];
        if (NRS.defaultModules['marketplace'] === false) delete NRS.transactionTypes[3];
        if (NRS.defaultModules['monetary_system'] === false) delete NRS.transactionTypes[5];
        if (NRS.defaultModules['data_cloud'] === false) delete NRS.transactionTypes[6];
        if (NRS.defaultModules['shuffling'] === false) delete NRS.transactionTypes[7];
        */
    	//console.log("suse: NRS.transactionTypes=");
