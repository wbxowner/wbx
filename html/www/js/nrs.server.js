    ...
    NRS.sendRequest = function (requestType, data, callback, options) {
        ...
        try {
            var winFields = [
                ...
                ["priceWIN", "priceNQT"],
                ["refundWIN", "refundNQT"],
                ["discountWIN", "discountNQT"],
                ["phasingQuorumWIN", "phasingQuorum"],
                ["phasingMinBalanceWIN", "phasingMinBalance"],
                ["controlQuorumWIN", "controlQuorum"],
                ["controlMinBalanceWIN", "controlMinBalance"],
                ["controlMaxFeesWIN", "controlMaxFees"],
                ["minBalanceWIN", "minBalance"],
                ["shufflingAmountWIN", "amount"],
                ["monitorAmountWIN", "amount"],
                ["monitorThresholdWIN", "threshold"]
            ];
            ...
            if (maxFees > 0 && new BigInteger(data.feeNQT).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                callback({
                    ...
                    "errorDescription": $.t("error_fee_exceeds_max_account_control_fee", {
                        "maxFee": NRS.convertToWIN(phasingControl.maxFees)
                    })
                });
                return;
            }
        ...
        if (requestType == "getAccountId") {
            ...
            var wbxAddress = new WbxAddress();
            var accountRS = "";
            if (wbxAddress.set(accountId)) {
                accountRS = wbxAddress.toString();
            }
            callback({
                "account": accountId,
                "accountRS": accountRS
