...
var NRS = (function(NRS, $, undefined) {
  // barton 06-27-2017
  var greenColor = '#72AB97', redColor = '#D47E6A', blackColor = '#A0A0A0'; 
  var phasedSuccessColor = greenColor, phasedWarningColor = '#f39c12', redColor = '#f56954';
					...
					NRS.sendRequest("getPhasingPollVote", {
						...
						if (finished) {
							if (responsePoll.approved) {
								state = "success";
								color = phasedSuccessColor;
							} else {
								state = "danger";
								color = phasedDangerColor;
							}
						} else {
							state = "warning";
							color = phasedWarningColor;
						}
						...
						var phasingDiv = "";
						phasingDiv += '<div class="show_popover" style="display:inline-block;min-width:94px;text-align:left;padding:3px;" '; // barton 7-1-2017
	 				 	phasingDiv += 'data-toggle="popover" data-container="body">';
						// phasingDiv += "<div class='label label-" + state + "' style='display:inline-block;margin-right:5px;'>" + icon + "</div>";  // barton 7-1-2017
						...
						if (vm == 1) {
							$popoverTypeTR.find("td:first").html($.t('accounts', 'Accounts') + ":");
							$popoverTypeTR.find("td:last").html(String(attachment.phasingWhitelist ? attachment.phasingWhitelist.length : ""));
							votesFormatted = NRS.convertToWIN(responsePoll.result) + " / " + NRS.convertToWIN(attachment.phasingQuorum) + " WBX";
							$popoverVotesTR.find("td:last").html(votesFormatted);
						}
						if (mbModel == 1) {
							if (minBalance > 0) {
								minBalanceFormatted = NRS.convertToWIN(minBalance) + " WBX";
    ...
    NRS.getTransactionRowHTML = function(t, actions, decimals) {
    	...
    	*/
		...
		if (receiving) {
			...
			feeColor = "color:" + blackColor + ";"; // barton 06-27-2017
		} else {
			...
			feeColor = "color:" + redColor + ";"; // barton 06-27-2017
		}
		var formattedAmount = "";
		if (amount != "") {
		...
		var amountColor = (sign == 1 ? "color:" + greenColor + ";" : (sign == -1 ? "color:" + redColor + ";" : "color:" + blackColor +  ";")); // barton 06-27-2017
    ...
    html += "<td style='vertical-align:middle;text-align:center;'>" + (hasMessage ? "&nbsp; <i class='fa fa-envelope-o'></i>&nbsp;" : "&nbsp;") + "</td>";
		html += '<td style="vertical-align:middle;">';
		// html += NRS.getTransactionIconHTML(t.type, t.subtype) + '&nbsp; ';  // barton 06-28-2017
    ...
    NRS.getLedgerEntryRow = function(entry, decimalParams) {
		...
		var color = "";
        if (entry.change > 0) {
			color = "color:" + greenColor + ";"; // barton 06-27-2017
		} else if (entry.change < 0) {
			color = "color:" + redColor + ";"; // barton 06-27-2017
			sign = "-";
        }
	...
	NRS.setup.transactions = function() {
		...
		var options = {
			"id": sidebarId,
			"titleHTML": ' <span data-i18n="dashboard">Dashboard</span>', // removed <i class="fa fa-dashboard"></i>  barton 7-1-2017
			"page": 'dashboard',
			"desiredPosition": 10
		};
