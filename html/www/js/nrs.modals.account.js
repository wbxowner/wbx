...
var NRS = (function(NRS, $) {
	...
	NRS.processAccountModalData = function(account) {
		if (account.unconfirmedBalanceNQT == "0") {
			...
			$("#user_info_modal_account_balance").html(NRS.formatAmount(account.unconfirmedBalanceNQT) + " WBX");
		}
        ...
        NRS.sendRequest("getBlockchainTransactions", {
			...
			if (response.transactions && response.transactions.length) {
				...
				for (var i = 0; i < response.transactions.length; i++) {
                    ...
                    var receiving;
					if (/^WBX\-/i.test(String(NRS.userInfoModal.user))) {
					...
					rows += "<tr>" +
						...
						"<td>" + transactionType + "</td>" +  // removed (+ NRS.getTransactionIconHTML(transaction.type, transaction.subtype) + "&nbsp")  barton 7-1-2017
						"<td class='numeric'  " + (transaction.type == 0 && receiving ? " style='color:#72AB97;'" : (!receiving && transaction.amount > 0 ? " style='color:#D47E6A'" : "")) + ">" + (!receiving && transaction.amount > 0 ? "-" : "")  + "" + NRS.formatAmount(transaction.amount, false, false, amountDecimals) + "</td>" +
						"<td class='numeric' " + (!receiving ? " style='color:#D47E6A'" : "") + ">" + NRS.formatAmount(transaction.fee, false, false, feeDecimals) + "</td>" +
		...
		NRS.sendRequest("getDGSGoods", {
			...
			if (response.goods && response.goods.length) {
				for (var i = 0; i < response.goods.length; i++) {
					...
					rows += "<tr><td><a href='#' data-goto-goods='" + NRS.escapeRespStr(good.goods) + "' data-seller='" + NRS.escapeRespStr(NRS.userInfoModal.user) + "'>" + NRS.escapeRespStr(good.name) + "</a></td><td class='numeric'>" + NRS.formatAmount(good.priceNQT, false, false, priceDecimals) + " WBX</td><td class='numeric'>" + NRS.format(good.quantity, false, quantityDecimals) + "</td></tr>";
				}
			}
