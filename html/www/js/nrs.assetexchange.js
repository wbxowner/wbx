...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.forms.addAssetBookmark = function ($modal) {
        ...
        if (!/^\d+$/.test(data.id) && !/^WBX\-/i.test(data.id)) {
            return {
                "error": $.t("error_asset_or_account_id_invalid")
            };
        }

        if (/^WBX\-/i.test(data.id)) {
    ...
    NRS.loadAsset = function (asset, refresh) {
        ...
        if (!refresh) {
            ...
            $("#sell_asset_for_wbx").html($.t("sell_asset_for_wbx", {
                "assetName": NRS.escapeRespStr(asset.name)
            }));
            $("#buy_asset_with_wbx").html($.t("buy_asset_with_wbx", {
                "assetName": NRS.escapeRespStr(asset.name)
            }));
        ...
        if (NRS.accountInfo.unconfirmedBalanceNQT == "0") {
            $("#your_wbx_balance").html("0");
            $("#buy_automatic_price").addClass("zero").removeClass("nonzero");
        } else {
            $("#your_wbx_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceNQT));
    ...
    $("#asset_exchange_bid_orders_table tbody, #asset_exchange_ask_orders_table tbody").on("click", "td", function (e) {
        ...
        try {
            ...
            $("#" + type + "_asset_total").val(NRS.convertToWIN(totalNQT));
        } catch (err) {
            return;
        }
    ...
    $("#sell_automatic_price, #buy_automatic_price").on("click", function () {
        try {
            ...
            if (price.cmp(new Big("0")) <= 0) {
                ...
                assetPrice.val(NRS.convertToWIN(price.toString()));
            }
            ...
            assetTotal.val(NRS.convertToWIN(total.toString()));
            assetTotal.css({
                "background": "",
                "color": ""
            });
    ...
    $("#asset_order_modal").on("show.bs.modal", function (e) {
        ...
        try {
            ...
            var priceNQT = new BigInteger(NRS.calculatePricePerWholeQNT(NRS.convertToNQT(String($("#" + orderType + "_asset_price").val())), currentAsset.decimals));
            var totalWIN = NRS.formatAmount(NRS.calculateOrderTotalNQT(quantityQNT, priceNQT, currentAsset.decimals), false, true);
        } catch (err) {
        ...
        if (orderType == "buy") {
            description = $.t("buy_order_description", {
                ...
                "wbx": NRS.formatAmount(priceNQTPerWholeQNT)
            });
            tooltipTitle = $.t("buy_order_description_help", {
                "wbx": NRS.formatAmount(priceNQTPerWholeQNT, false, true),
                "total_wbx": totalWIN
            });
        } else {
        ...
        $("#asset_order_total").html(totalWIN + " WBX");
    ...
    NRS.setup.asset_exchange = function () {
        ...
        var options = {
            "id": sidebarId,
            "titleHTML": '<span data-i18n="asset_exchange">Asset Exchange</span>',  // removed <i class="fa fa-signal"></i>  barton 7-1-2017
            "page": 'asset_exchange',
            "desiredPosition": 30,
        ...
        NRS.appendMenuItemToTSMenuItem(sidebarId, options);*/  // barton 07-07-2017
        options = {
