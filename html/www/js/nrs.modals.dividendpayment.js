...
var NRS = (function(NRS, $, undefined) {
    ...
    $("#dividend_payment_amount_per_share, #dividend_payment_height").on("blur", function() {
        ...
        var amountWINPerShare = $modal.find("#dividend_payment_amount_per_share").val();
        ...
        if (amountWINPerShare && /^\d+$/.test(height)) {
            NRS.getAssetAccounts(NRS.getCurrentAsset().asset, height,
                function (response) {
                    ...
                    var priceNQT = new BigInteger(NRS.calculatePricePerWholeQNT(NRS.convertToNQT(amountWINPerShare), NRS.getCurrentAsset().decimals));
                    var totalWIN = NRS.calculateOrderTotal(totalQuantityQNT, priceNQT);
