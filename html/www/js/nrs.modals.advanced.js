...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.initAdvancedModalFormValues = function($modal) {
		...
		context = {
			...
			inputName: "phasingQuorumWIN",
			addonText: "WBX",
			addonI18n: "wbx_unit"
		};
		$elems = NRS.initModalUIElement($modal, '.approve_transaction_amount_wbx', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		context['inputName'] = 'phasingMinBalanceWIN';
		context['addonText'] = 'WBX';
		context['addonI18n'] = 'wbx_unit';
		$elems = NRS.initModalUIElement($modal, '.approve_min_balance_wbx', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
