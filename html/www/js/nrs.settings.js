...
var NRS = (function(NRS, $) {
  ...
  /*NRS.defaultModules = {
		"asset_exchange": true,
		"send_token" : true,
		"monetary_system": true,
		"polls": true,
		"data_cloud": true,
		"marketplace": true,
		"shuffling": true,
		"messages": true,
		"transactions_notification": true,
		"aliases": true,
		"plugins": true,
		'issue_asset': true,
		'add_asset_button': true
	};*/
	...
	NRS.defaultSettings = {
		...
		"items_page": "15",
		"admin_password": "",
        "exchange_url": "https://wbx-exchange.domain.ext/",
	...
	NRS.languages = {
		...
		// "ru": "Русский",                 // russian
		"zh-cn": "简体 simplified",      // chinese simplified
		"zh-tw": "繁体 traditional"      // chinese traditional
		///suse
	};
	...
	NRS.pages.settings = function() {
		...
		for (var key in NRS.settings) {
            ...
            if (/_warning/i.test(key) && key != "asset_transfer_warning" && key != "currency_transfer_warning") {
				if (setting.length) {
					setting.val(NRS.convertToWIN(NRS.settings[key]));
				}
    ...
    // barton 7-16-2017: hide this select widget and show buttons instead
    var lang = NRS.settings['language'];
    $('.lang-btns').remove();
    $langSelBoxes.css({ display: 'none' });
    $langSelBoxes.after('<div style="margin-left: 10px; margin-top: 5px; padding: 1px; background: #2c2c2c;"><div class="lang-btns btn-group" data-toggle="buttons"></div></div>');
    var $langButtons = $('.lang-btns');
    $.each(NRS.languages, function(code, name) {
			$langButtons.append('<label class="btn btn-default btn-xs' + (code == lang ? ' active' : '') + '"><input type="radio" name="options" value="' + code + '" autocomplete="off" ' + (code == lang ? ' checked' : '') + ' /> ' + name + '</label>');
		});
    $langButtons.val(NRS.settings['language']);
    $('.lang-btns input[type=radio]').on('change', function() {
      $langSelBoxes.val(this.value).change();
    });
	};
