	...
	NRS.pages.news = function() {
      ...
      var settingsReddit = {
         ...
         "filter": function(entry, tokens) {
            return tokens.title.indexOf("Donations toward") == -1 && tokens.title.indexOf("WBX tipping bot has arrived") == -1
         },
         "ssl": ssl
      };

      $("#winforum_news").rss("http://forum.winbit.com/index.php?action=.xml;type=rss", settings, NRS.newsLoaded);
      //$("#reddit_news").rss("http://www.reddit.com/r/WBX/.rss", settingsReddit, NRS.newsLoaded);
      //$("#wincoin_blogspot_news").rss("http://wincoin.blogspot.com/feeds/posts/default", settings, NRS.newsLoaded);
      //$("#winer_news").rss("http://winer.org/feed/", settings, NRS.newsLoaded);
		NRS.pageLoaded();
	};
